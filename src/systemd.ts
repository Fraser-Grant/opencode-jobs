...
function extractServePortLines(): string[] {
  return [
    "extract_serve_port() {",
    String.raw`  sed -n 's/.*listening on http:\/\/127\.0\.0\.1:\([0-9][0-9]*\).*/\1/p' "$1" | head -n 1`,
    "}",
  ];
}

function healthcheckLines(): string[] {
  return [
    "check_health() {",
    ...
    '  health_out="$(mktemp)"',
    '  health_err="$(mktemp)"',
    '  "$oc_bin" serve --port 0 >"$health_out" 2>"$health_err" &',
    "  health_pid=$!",
    '  health_port="$(extract_serve_port "$health_out")"',
    "  health_tries=0",
    '  while [ "$health_tries" -lt 100 ] && [ -z "$health_port" ]; do',
    ...
    '    if curl -s -o /dev/null --max-time 2 "http://127.0.0.1:$health_port/global/health"; then',
...
function compactSessionLines(): string[] {
  return [
    ...
    '    serve_port="$(extract_serve_port "$serve_out")"',
...
export function runScriptContent(
  ...
  const stallWindow = isTracked ? (job.stallTimeoutSeconds ?? 300) : 0;
  const isStallEnabled = stallWindow > 0;
  return [
    "#!/bin/sh",
    "set -u",
    ...
    '  exec "$oc_bin" "$@"',
    "}",
    ...(isCompact || isStallEnabled ? extractServePortLines() : []),
    ...(isStallEnabled ? healthcheckLines() : []),
    ...(isCompact ? compactSessionLines() : []),
    ...(isTracked
      ? [
          'json_out="$(mktemp)"',
          `stall_window=${String(stallWindow)}`,
          "stall_ticks=$((stall_window * 2))",
          "max_stall_resumes=2",
          ...
          "  stalled=0",
          '  : >"$json_out"',
          '  run_opencode "$resume_session" 1 >"$json_out" 2>&1 &',
          "  oc_pid=$!",
          "  last_size=0",
          "  idle=0",
          '  if [ "$stall_ticks" -gt 0 ]; then',
          '    while kill -0 "$oc_pid" 2>/dev/null; do',
          "      sleep 0.5",
          '      cur_size="$(wc -c <"$json_out" 2>/dev/null)" || cur_size=0',
          ...
          "      fi",
          "      idle=$((idle + 1))",
          '      if [ "$idle" -lt "$stall_ticks" ]; then continue; fi',
          '      hc_result="$(check_health)"',
          '      case "$hc_result" in',
          "        dead) break ;;",
          "        healthy|alive)",
          "          stalled=1",
          ...
          "          ;;",
          "        *)",
          String.raw`          printf '{"runId":"%s","slug":"%s","scopeId":"%s","startedAt":%s,"event":"healthcheck","result":"unhealthy","idleSeconds":%s}\n' "$run_id" "$slug" "$scope" "$started" "$((idle / 2))" >> "$record_file"`,
          "          idle=0",
          "          ;;",
          "      esac",
          "    done",
          '    if [ "$stalled" -eq 1 ]; then',
          '      if kill "$oc_pid" 2>/dev/null; then',
          "        grace=0",
          '        while kill -0 "$oc_pid" 2>/dev/null && [ "$grace" -lt 10 ]; do',
          "          sleep 1",
          "          grace=$((grace + 1))",
          "        done",
          '        kill -9 "$oc_pid" 2>/dev/null',
          '        echo "opencode-jobs: no events for $((idle / 2))s (healthcheck: $hc_result), killing stalled attempt"',
          "      else",
          "        stalled=0",
          "      fi",
          "    fi",
          ...
          "  code=$?",
          '  cat "$json_out"',
          `  seen_session="$(sed -n '${SESSION_ID_SED}' "$json_out" | head -n 1)"`,
          '  if [ -n "$seen_session" ]; then new_session="$seen_session"; fi',
          '  if [ "$stalled" -eq 1 ]; then',
          "    attempt=$((attempt + 1))",
          String.raw`    printf '{"runId":"%s","slug":"%s","scopeId":"%s","startedAt":%s,"event":"stall","attempt":%s,"idleSeconds":%s,"healthcheck":"%s"}\n' "$run_id" "$slug" "$scope" "$started" "$attempt" "$((idle / 2))" "$hc_result" >> "$record_file"`,
          '    if [ "$attempt" -gt "$max_stall_resumes" ]; then',
          String.raw`      printf '{"runId":"%s","slug":"%s","scopeId":"%s","startedAt":%s,"event":"resume-exhausted","attempts":%s}\n' "$run_id" "$slug" "$scope" "$started" "$attempt" >> "$record_file"`,
          "      code=125",
          "      break",
          "    fi",
          '    if [ -n "$new_session" ]; then resume_session="$new_session"; fi',
          String.raw`    printf '{"runId":"%s","slug":"%s","scopeId":"%s","startedAt":%s,"event":"resume","attempt":%s,"sessionId":"%s"}\n' "$run_id" "$slug" "$scope" "$started" "$attempt" "$resume_session" >> "$record_file"`,
          "    continue",
          "  fi",
          '  if [ "$code" -ne 0 ] && [ "$fresh_retry" -eq 0 ] && [ -n "$prev_session" ] && [ -z "$new_session" ] && grep -qi "session not found" "$json_out"; then',
      ...
      : ['run_opencode "" 0 &', "oc_pid=$!", 'wait "$oc_pid"', "code=$?"]),
    "trap - TERM INT",
